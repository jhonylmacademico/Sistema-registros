...
const MULTICENTROS = [
  { id: 'admin', nombre: 'Centro Administrativo', clase: 'border-l-blue-500 bg-blue-50' },
  { id: 'norte', nombre: 'Sucursal Norte', clase: 'border-l-green-500 bg-green-50' },
  { id: 'sur', nombre: 'Sucursal Sur', clase: 'border-l-yellow-500 bg-yellow-50' },
  { id: 'almacen', nombre: 'Almacén Principal', clase: 'border-l-red-500 bg-red-50' },
  { id: 'dist', nombre: 'Centro de Distribución', clase: 'border-l-purple-500 bg-purple-50' }
];

const datosIniciales = [
  { id: '1', centro: 'admin', numero: '0001', tipo: 'Laptop', nombreEquipo: 'LAP-JPEREZ', marca: 'Lenovo', modelo: 'ThinkPad T14', codigoActivo: 'AF-2024-001', numeroSerie: 'LP-001-ABC', procesador: 'Intel Core i5', generacion: '10ma', ram: '8 GB', tipoDisco: 'SSD M.2', capacidadDisco: '256 GB', sistemaOperativo: 'Windows 11 Pro', mac: 'AA:BB:CC:DD:EE:01', ip: '192.168.1.101', estado: 'Activo', piso: '2', area: 'Contabilidad', personaAsignada: 'Juan Pérez', nombreResponsable: 'Juan Pérez', fechaAsignacion: '2024-01-15', notas: '' },
  { id: '2', centro: 'admin', numero: '0002', tipo: 'Impresora', marca: 'HP', modelo: 'LaserJet Pro', numeroSerie: 'IMP-002-XYZ', estado: 'Activo', piso: '1', area: 'Recursos Humanos', oficina: 'Recursos Humanos', fechaAsignacion: '2024-01-10', notas: 'Cartucho nuevo' },
  { id: '3', centro: 'admin', numero: '0003', tipo: 'Computadora de Escritorio', nombreEquipo: 'PC-MGARCIA', marca: 'Dell', modelo: 'Optiplex 7090', codigoActivo: 'AF-2024-003', numeroSerie: 'PC-003-DEF', procesador: 'Intel Core i7', generacion: '11va', ram: '16 GB', tipoDisco: 'SSD M.2', capacidadDisco: '512 GB', sistemaOperativo: 'Windows 10 Pro', mac: 'AA:BB:CC:DD:EE:03', ip: '192.168.1.103', estado: 'Dañado', piso: '3', area: 'Ventas', personaAsignada: 'María García', nombreResponsable: 'María García', fechaAsignacion: '2024-02-20', marcaMonitor: 'Dell', modeloMonitor: 'P2422H', marcaCPU: 'Dell', modeloCPU: 'Optiplex SFF', notas: 'Falla fuente de poder' }
];
...
export default function App() {
  ...
  useEffect(() => {
    const d = localStorage.getItem('activos_fijos_v3');
    if (d) { setActivos(JSON.parse(d)); } else { setActivos(datosIniciales); localStorage.setItem('activos_fijos_v3', JSON.stringify(datosIniciales)); }
    const p = localStorage.getItem('app_pass_v3');
    if (p) setCustomPass(p);
  }, []);

  const guardarDatos = (n) => { setActivos(n); localStorage.setItem('activos_fijos_v3', JSON.stringify(n)); };
  const handleLogin = (e) => { e.preventDefault(); if (user === 'admin' && pass === customPass) setIsLoggedIn(true); };
  
  const getNextNumber = () => {
    const d = JSON.parse(localStorage.getItem('activos_fijos_v3') || '[]');
  ...
  const exportarCSV = () => {
    ...
    const h = ['N°', 'Tipo', 'Nombre Equipo', 'Marca', 'Modelo', 'Cod. Activo', 'N° Serie', 'Procesador', 'Gen', 'RAM', 'Tipo Disco', 'Cap. Disco', 'S.O.', 'MAC', 'IP', 'Estado', 'Piso', 'Área', 'Asignado A', 'Responsable', 'Fecha Asig.', 'Mar. Monitor', 'Mod. Monitor', 'Mar. CPU', 'Mod. CPU', 'Oficina', 'Notas'];
    const rows = datosC.map(a => [a.numero, a.tipo, a.nombreEquipo||'', a.marca||'', a.modelo||'', a.codigoActivo||'', a.numeroSerie||'', a.procesador||'', a.generacion||'', a.ram||'', a.tipoDisco||'', a.capacidadDisco||'', a.sistemaOperativo||'', a.mac||'', a.ip||'', a.estado, a.piso||'', a.area||'', a.personaAsignada||'', a.nombreResponsable||'', a.fechaAsignacion||'', a.marcaMonitor||'', a.modeloMonitor||'', a.marcaCPU||'', a.modeloCPU||'', a.oficina||'', a.notas||'']);
    const csv = '\uFEFF' + [h.join(';'), ...rows.map(r => r.map(c => '"'+String(c).replace(/"/g,'""')+'"').join(';'))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a'); link.href = url; 
    link.download = 'Reporte_' + (MULTICENTROS.find(c=>c.id===centroActual)?.nombre || 'General').replace(/\s+/g, '_') + '.csv';
  ...
  const activosFiltrados = datosFinales.filter(a => (a.marca||'').toLowerCase().includes(busqueda.toLowerCase()) || (a.nombreEquipo||'').toLowerCase().includes(busqueda.toLowerCase()) || (a.personaAsignada||'').toLowerCase().includes(busqueda.toLowerCase()) || (a.numeroSerie||'').toLowerCase().includes(busqueda.toLowerCase()) || (a.numero||'').includes(busqueda));
    ...
    <div className='min-h-screen bg-gray-100 pb-20'>
      ...
      <div className='bg-blue-700 text-white p-4 shadow-lg flex justify-between items-center sticky top-0 z-10'>
        <div className='flex gap-3 items-center'>
          ...
          <h1 className='text-lg font-bold'>{centroActual ? MULTICENTROS.find(c => c.id === centroActual)?.nombre : 'Multicentros'}</h1>
        </div>
      ...
      <div className='p-4'>
        ...
        {vista === 'hub' && (
          <div className='space-y-4'>
            <div className='flex items-center gap-2 text-gray-600 mb-2'><Building2 size={20} /><h2 className='text-xl font-bold text-gray-800'>Seleccionar Centro</h2></div>
            <div className='grid grid-cols-1 gap-4'>
              {MULTICENTROS.map(c => {
                const count = activos.filter(a => a.centro === c.id).length;
                return (
                  <button key={c.id} onClick={() => { setCentroActual(c.id); setVista('dashboard'); setBusqueda(''); }} className={'p-6 rounded-xl shadow-sm border-l-4 text-left active:bg-gray-200 transition ' + c.clase}>
        ...
        {centroActual && vista === 'lista' && (
          <div>
            ...
            <div className='space-y-3'>
              {activosFiltrados.length === 0 ? <p className='text-center text-gray-500 mt-10'>Sin activos registrados.</p> : 
...
function ConfigVista({ customPass, setCustomPass, setVista, setMsg }) {
  ...
  const handleChange = (e) => {
    ...
    setCustomPass(nueva); localStorage.setItem('app_pass_v3', nueva);
...
function FormularioActivo({ activo, guardarDatos, setVista, getNextNumber, centroActual }) {
  const [form, setForm] = useState(activo || { id: Date.now().toString(), centro: centroActual, numero: getNextNumber(), tipo: 'Laptop', nombreEquipo: '', marca: '', modelo: '', codigoActivo: '', numeroSerie: '', procesador: '', generacion: '', ram: '', tipoDisco: 'SSD M.2', capacidadDisco: '', sistemaOperativo: '', mac: '', ip: '', estado: 'Activo', piso: '', area: '', personaAsignada: '', nombreResponsable: '', fechaAsignacion: '', marcaMonitor: '', modeloMonitor: '', marcaCPU: '', modeloCPU: '', oficina: '', notas: '' });

  // --- SOLUCIÓN AL ERROR DEL FORMULARIO ---
  // Ahora lee el tipo directamente del estado 'form', no del dato original
  const esPC = form.tipo === 'Computadora de Escritorio';
  const esImpSw = form.tipo === 'Impresora' || form.tipo === 'Switch';

  const h = (e) => setForm({ ...form, [e.target.name]: e.target.value });
  
  const handleSubmit = (e) => {
    e.preventDefault();
    const datos = JSON.parse(localStorage.getItem('activos_fijos_v3') || '[]');
        ...
        {esImpSw ? (
          ...
          <>
            <div className='bg-blue-50 p-3 rounded-lg border-l-4 border-blue-400 space-y-3'>
              ...
              <div className='grid grid-cols-2 gap-3'>
                <div><label className='block text-xs font-medium text-gray-700 mb-1'>Nombre Máquina</label><input name='nombreEquipo' value={form.nombreEquipo||''} onChange={h} required className='w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm' placeholder='LAP-CONT-01' /></div>
                <div><label className='block text-xs font-medium text-gray-700 mb-1'>Estado</label><select name='estado' value={form.estado} onChange={h} className='w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm'><option>Activo</option><option>En Mantenimiento</option><option>Dañado</option></select></div>
            ...
            <div className='bg-purple-50 p-3 rounded-lg border-l-4 border-purple-400 space-y-3'>
              ...
              <div className='grid grid-cols-2 gap-3'>
                ...
                <div><label className='block text-xs font-medium text-gray-700 mb-1'>Capacidad Disco</label><input name='capacidadDisco' value={form.capacidadDisco||''} onChange={h} className='w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm' placeholder='256 GB' /></div>
