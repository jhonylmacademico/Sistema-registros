...
export default function App() {
  ...
  useEffect(() => {
    const c = localStorage.getItem('mis_centros_v71'); if (c) setCentros(JSON.parse(c)); else { setCentros(CENTROS_DEFAULT); localStorage.setItem('mis_centros_v71', JSON.stringify(CENTROS_DEFAULT)); }
    const d = localStorage.getItem('activos_fijos_v71'); if (d) setActivos(JSON.parse(d)); else { setActivos(datosIniciales); localStorage.setItem('activos_fijos_v71', JSON.stringify(datosIniciales)); }
    const o = localStorage.getItem('mis_oficinas_v71'); if (o) setOficinas(JSON.parse(o)); else { setOficinas(OFICINAS_DEFAULT); localStorage.setItem('mis_oficinas_v71', JSON.stringify(OFICINAS_DEFAULT)); }
    const p = localStorage.getItem('app_pass_v71'); if (p) setCustomPass(p);
  }, []);

  const guardarDatos = (n) => { setActivos(n); localStorage.setItem('activos_fijos_v71', JSON.stringify(n)); };
  const handleLogin = (e) => { e.preventDefault(); if (user === 'admin' && pass === customPass) setIsLoggedIn(true); };
  const getNextNumber = () => { const d = JSON.parse(localStorage.getItem('activos_fijos_v71') || '[]'); return (d.reduce((m, a) => Math.max(m, parseInt(a.numero || '0')), 0) + 1).toString().padStart(4, '0'); };
  const agregarCentro = () => { const n = prompt('Nombre del nuevo Multicentro:'); if (n && n.trim()) { const nc = [...centros, { id: n.trim().toLowerCase().replace(/\s+/g, '_'), nombre: n.trim() }]; setCentros(nc); localStorage.setItem('mis_centros_v71', JSON.stringify(nc)); } };
  ...
  const gestionarOficinas = () => {
    ...
    if (accionUpper.startsWith('NUEVA:')) {
      ...
      if(nombre) { nuevas.push({ id: nombre.toLowerCase().replace(/\s+/g, '_'), nombre }); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v71', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina agregada'); }
    } else if (accionUpper.startsWith('EDITAR:')) {
      const partes = accion.substring(7).trim().split(' ');
      const idx = parseInt(partes[0]) - 1;
      if (!isNaN(idx) && idx >= 0 && idx < nuevas.length) { nuevas[idx].nombre = partes.slice(1).join(' '); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v71', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina editada'); }
    } else if (accionUpper.startsWith('BORRAR:')) {
      const idx = parseInt(accion.substring(7).trim()) - 1;
      if (!isNaN(idx) && idx >= 0 && idx < nuevas.length) { nuevas.splice(idx, 1); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v71', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina borrada'); }
...
function ConfigVista({ customPass, setCustomPass, setVista, setMsg }) {
  ...
  const h = (e) => { e.preventDefault(); if (nueva.length < 4) { alert('Mínimo 4 caracteres'); return; } setCustomPass(nueva); localStorage.setItem('app_pass_v71', nueva); setMsg('Contraseña actualizada'); setTimeout(() => setMsg(''), 3000); setVista('hub'); };
...
function FormularioActivo({ activo, guardarDatos, setVista, getNextNumber, centroActual, oficinasCentro }) {
  ...
  const handleSubmit = (e) => { e.preventDefault(); const datos = JSON.parse(localStorage.getItem('activos_fijos_v71') || '[]'); if (activo) { guardarDatos(datos.map(a => a.id === activo.id ? form : a)); } else { guardarDatos([...datos, form]); } setVista('lista'); };
  const handleEliminar = () => { if (confirm('¿Eliminar?')) { const datos = JSON.parse(localStorage.getItem('activos_fijos_v71') || '[]'); guardarDatos(datos.filter(a => a.id !== form.id)); setVista('lista'); } };
