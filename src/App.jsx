...
export default function App() {
  ...
  // Lógica Mantenimiento Preventivo
  const hoy = new Date();
      ...
      <div className='p-4'>
        ...
        {centroActual && vista === 'dashboard' && (
          <div className='space-y-4'>
            ...
            {pisosCentroActual.length === 0 ? <p className='text-sm text-gray-500 bg-white p-4 rounded-lg text-center'>No hay oficinas asignadas a pisos. Ve a Gestionar.</p> : (
        ...
        {centroActual && vista === 'oficinas' && (
          <div>
            ...
            <div className='bg-white p-4 rounded-xl shadow-sm mb-4'><div className='flex justify-between items-center mb-4'><h2 className='font-bold text-lg text-gray-800 flex items-center gap-2'><Layers size={20} /> Pisos de {centros.find(c=>c.id===centroActual)?.nombre}</h2>{esAdmin && <button onClick={agregarPiso} className='bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-1 font-bold text-sm'><Plus size={18} /> Nuevo Piso</button>}</div>{pisosCentroActual.length === 0 ? <p className='text-gray-500 text-center py-4 text-sm'>No hay pisos creados en este centro.</p> : (<div className='flex flex-wrap gap-2'>{pisosCentroActual.map((p, index) => (<div key={index} className='flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg border border-gray-200'><span className='font-bold text-gray-700 text-sm'>Piso {p}</span>{esAdmin && <button onClick={() => { if (confirm('Eliminar el piso ' + p + '?')) { const np = pisosCentroActual.filter((_, i) => i !== index); guardarPisos({...pisos, [centroActual]: np}); const no = oficinasCentro.map(o => o.piso === p ? {...o, piso: ''} : o); guardarOficinas({...oficinas, [centroActual]: no}); setMsg('Piso eliminado'); setTimeout(()=>setMsg(''), 2000); } }} className='text-red-500'><Trash2 size={14} /></button>}</div>))}</div>)}</div>
        ...
        {vista === 'config' && esAdmin && <ConfigVista setVista={setVista} setMsg={setMsg} setActivos={setActivos} setCentros={setCentros} setOficinas={setOficinas} setPisos={setPisos} setCustomPass={setCustomPass} setLogo={setLogo} guardarArchivoNativo={guardarArchivoNativo} />}
      </div>
    ...
    <div className='bg-green-50 p-3 rounded-lg border-l-4 border-green-400 space-y-3'>
      ...
      {!form.enAlmacen ? (
        <div className='grid grid-cols-2 gap-3'>
          ...
          <div className='col-span-2'><label className='block text-xs font-medium text-gray-700 mb-1'>Fecha Asignacion</label><input type='date' name='fechaAsignacion' value={form.fechaAsignacion||''} onChange={h} className='w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm' /></div>
          <div className='col-span-2'><label className='block text-xs font-medium text-gray-700 mb-1'>Próx. Mantenimiento</label><input type='date' name='fechaProxMantenimiento' value={form.fechaProxMantenimiento||''} onChange={h} className='w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm' /></div>
        </div>
      ) : (
        <div className='grid grid-cols-2 gap-3'>
