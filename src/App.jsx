...
const CAMPOS_DISPONIBLES = [
  { key: 'numero', label: 'N° Registro' }, { key: 'tipo', label: 'Tipo Equipo' }, { key: 'nombreEquipo', label: 'Nombre Equipo' }, { key: 'marca', label: 'Marca (Gen/Imp)' }, { key: 'marcaCPU', label: 'Marca CPU' }, { key: 'procesador', label: 'Procesador' }, { key: 'ram', label: 'RAM' }, { key: 'numeroSerie', label: 'N° Serie' }, { key: 'estado', label: 'Estado' }, { key: 'enAlmacen', label: 'En Almacén' }, { key: 'oficina', label: 'Oficina / Área' }, { key: 'piso', label: 'Piso' }, { key: 'personaAsignada', label: 'Persona Asignada' }, { key: 'numeroEmpleado', label: 'N° Empleado' }
];
...
const datosIniciales = [
  ...
  { id: '3', centro: 'sucre', numero: '0003', tipo: 'Laptop', marca: 'Dell', modelo: 'Latitude', procesador: 'Intel i7', ram: '16 GB', tipoDisco: 'SSD M.2', capacidadDisco: '512 GB', tipoDisco2: 'Ninguno', capacidadDisco2: '', estado: 'Dañado', enAlmacen: false, oficina: 'Contabilidad', piso: '2', personaAsignada: 'Ana Torres', nombreResponsable: 'Ana Torres', fechaAsignacion: '2024-02-20', notas: 'Pantalla rota' }
];
...
export default function App() {
  ...
  useEffect(() => {
    const c = localStorage.getItem('mis_centros_v7'); if (c) setCentros(JSON.parse(c)); else { setCentros(CENTROS_DEFAULT); localStorage.setItem('mis_centros_v7', JSON.stringify(CENTROS_DEFAULT)); }
    const d = localStorage.getItem('activos_fijos_v7'); if (d) setActivos(JSON.parse(d)); else { setActivos(datosIniciales); localStorage.setItem('activos_fijos_v7', JSON.stringify(datosIniciales)); }
    const o = localStorage.getItem('mis_oficinas_v7'); if (o) setOficinas(JSON.parse(o)); else { setOficinas(OFICINAS_DEFAULT); localStorage.setItem('mis_oficinas_v7', JSON.stringify(OFICINAS_DEFAULT)); }
    const p = localStorage.getItem('app_pass_v7'); if (p) setCustomPass(p);
  }, []);

  const guardarDatos = (n) => { setActivos(n); localStorage.setItem('activos_fijos_v7', JSON.stringify(n)); };
  const handleLogin = (e) => { e.preventDefault(); if (user === 'admin' && pass === customPass) setIsLoggedIn(true); };
  const getNextNumber = () => { const d = JSON.parse(localStorage.getItem('activos_fijos_v7') || '[]'); return (d.reduce((m, a) => Math.max(m, parseInt(a.numero || '0')), 0) + 1).toString().padStart(4, '0'); };
  const agregarCentro = () => { const n = prompt('Nombre del nuevo Multicentro:'); if (n && n.trim()) { const nc = [...centros, { id: n.trim().toLowerCase().replace(/\s+/g, '_'), nombre: n.trim() }]; setCentros(nc); localStorage.setItem('mis_centros_v7', JSON.stringify(nc)); } };
  ...
  const gestionarOficinas = () => {
    ...
    const accion = prompt(`OFICINAS DE ${centros.find(c=>c.id===centroActual)?.nombre}:\n\n${texto}\n\nEscribe:\n- NUEVA: [Nombre]\n- EDITAR: [Num] [Nuevo Nombre]\n- BORRAR: [Num]`);
    if (!accion) return;
    let nuevas = [...listaAct];
    if (accion.toUpperCase().startsWith('NUEVA:')) {
      const nombre = accion.substring(6).trim();
      if(nombre) { nuevas.push({ id: nombre.toLowerCase().replace(/\s+/g, '_'), nombre }); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v7', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina agregada'); }
    } else if (accion.toUpperCase().startsWith('EDITAR:')) {
      const partes = accion.substring(7).trim().split(' ');
      const idx = parseInt(partes[0]) - 1;
      if (!isNaN(idx) && idx >= 0 && idx < nuevas.length) { nuevas[idx].nombre = partes.slice(1).join(' '); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v7', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina editada'); }
    } else if (accion.toUpperCase().startsWith('BORRAR:')) {
      const idx = parseInt(accion.substring(7).trim()) - 1;
      if (!isNaN(idx) && idx >= 0 && idx < nuevas.length) { nuevas.splice(idx, 1); setOficinas({...oficinas, [centroActual]: nuevas}); localStorage.setItem('mis_oficinas_v7', JSON.stringify({...oficinas, [centroActual]: nuevas})); setMsg('Oficina borrada'); }
        ...
        {centroActual && vista === 'dashboard' && (
          <div className='space-y-4'>
            ...
            <div className='grid grid-cols-2 gap-3'>
              ...
              <button onClick={() => { setOficinaFiltro(null); setEstadoFiltro('Dañado'); setVista('lista'); }} className='bg-white p-4 rounded-xl shadow-sm border-l-4 border-red-500 text-left'><p className='text-gray-500 text-xs'>Dañados</p><p className='text-2xl font-bold text-red-600'>{datosCentro.filter(a=>a.estado==='Dañado').length}</p></button>
            ...
            <div className='flex justify-between items-center mt-4 mb-2'>
              <h3 className='font-bold text-gray-700 flex items-center gap-2'><MapPin size={18} /> Oficinas y Áreas</h3>
            ...
            {oficinasCentro.length === 0 ? <p className='text-sm text-gray-500 bg-white p-4 rounded-lg text-center'>No hay oficinas creadas. Toca "Gestionar" para agregar.</p> : (
              <div className='grid grid-cols-2 gap-3'>
                ...
                <div key={a.id} onClick={() => { setEditando(a); setVista('formulario'); }} className='bg-white p-4 rounded-xl shadow-sm border-l-4 border-blue-400 active:bg-gray-50'>
                  <div className='flex justify-between items-start'>
                    ...
                    <div className='text-right'>
                      {a.enAlmacen && <span className='text-xs font-bold px-2 py-1 rounded-full bg-indigo-100 text-indigo-700 block mb-1'>Almacén</span>}
            ...
            {camposSeleccionados.length > 0 && (
              <div className='bg-white p-4 rounded-xl shadow-sm overflow-x-auto'>
                ...
                <table className='w-full text-sm text-left text-gray-500 min-w-[500px]'>
                  <thead className='text-xs text-gray-700 uppercase bg-gray-50'><tr>{camposSeleccionados.map(k => <th key={k} className='px-2 py-2'>{CAMPOS_DISPONIBLES.find(c=>c.key===k)?.label}</th>)}</tr></thead>
...
function ConfigVista({ customPass, setCustomPass, setVista, setMsg }) {
  ...
  const h = (e) => { e.preventDefault(); if (nueva.length < 4) { alert('Mínimo 4 caracteres'); return; } setCustomPass(nueva); localStorage.setItem('app_pass_v7', nueva); setMsg('Contraseña actualizada'); setTimeout(() => setMsg(''), 3000); setVista('hub'); };
  return (<div className='bg-white p-6 rounded-xl shadow-sm'><button onClick={() => setVista('hub')} className='flex items-center text-blue-600 font-bold mb-4'><ArrowLeft size={20} /> Volver</button><h2 className='text-xl font-bold text-gray-800 mb-4 flex items-center gap-2'><Key size={24} /> Cambiar Contraseña</h2><form onSubmit={h} className='space-y-4'><input type='password' value={nueva} onChange={e => setNueva(e.target.value)} required className='w-full p-3 border border-gray-300 rounded-lg bg-gray-50' placeholder='Nueva contraseña' /><button type='submit' className='w-full bg-blue-600 text-white p-3 rounded-lg font-bold'>Guardar</button></form></div>);
}
...
function FormularioActivo({ activo, guardarDatos, setVista, getNextNumber, centroActual, oficinasCentro }) {
  ...
  const handleSubmit = (e) => { e.preventDefault(); const datos = JSON.parse(localStorage.getItem('activos_fijos_v7') || '[]'); if (activo) { guardarDatos(datos.map(a => a.id === activo.id ? form : a)); } else { guardarDatos([...datos, form]); } setVista('lista'); };
  const handleEliminar = () => { if (confirm('¿Eliminar?')) { const datos = JSON.parse(localStorage.getItem('activos_fijos_v7') || '[]'); guardarDatos(datos.filter(a => a.id !== form.id)); setVista('lista'); } };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className='bg-white p-4 rounded-xl shadow-sm space-y-4'>
        <h2 className='font-bold text-lg text-gray-800 border-b pb-2'>{activo ? 'Editar N° ' + form.numero : 'Nuevo Registro'}</h2>
  ...
  return (
    <div className='bg-green-50 p-3 rounded-lg border-l-4 border-green-400 space-y-3'>
      ...
      <label className='flex items-center gap-3 p-3 bg-white rounded-lg border-2 border-indigo-200 cursor-pointer'>
        ...
        <div>
          <span className='font-bold text-indigo-700'>Marcar como "En Almacén"</span>
          <p className='text-xs text-gray-500'>Sin asignar a ningún usuario.</p>
        </div>
      </label>
      ...
      {!form.enAlmacen ? (
        <div className='grid grid-cols-2 gap-3'>
          ...
          <OficinaSelect etiqueta="Oficina / Área" req={true} />
