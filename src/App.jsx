...
const COLORES_CENTROS = [
  { borde: 'border-blue-500', fondo: 'bg-blue-50' }, { borde: 'border-green-500', fondo: 'bg-green-50' }, { borde: 'border-amber-500', fondo: 'bg-amber-50' }, { borde: 'border-pink-500', fondo: 'bg-pink-50' }, { borde: 'border-purple-500', fondo: 'bg-purple-50' }, { borde: 'border-red-500', fondo: 'bg-red-50' }, { borde: 'border-indigo-500', fondo: 'bg-indigo-50' }, { borde: 'border-teal-500', fondo: 'bg-teal-50' }
];
...
const OFICINAS_DEFAULT = { sucre: [{ id: 'conta', nombre: 'Contabilidad', piso: '2' }] };
...
export default function App() {
  ...
  const [userRole, setUserRole] = useState('admin'); // 'admin' o 'auditor'
      ...
      img.onload = async () => {
        try {
          ...
          if (detectedCode) {
            ...
            const equipo = activos.find(a => a.codigoActivo === detectedCode || a.numeroSerie === detectedCode);
        ...
        <div className='bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm border border-gray-100'>
          ...
          <form onSubmit={handleLogin} className='space-y-4'>
            ...
            <div className='flex gap-2'>
              ...
              <button type='button' onClick={() => setUserRole('auditor')} className={'flex-1 p-2 rounded-lg text-sm font-bold ' + (userRole === 'auditor' ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-600')}>Auditor (Solo Lectura)</button>
            </div>
  ...
  // Lógica de Mantenimiento Preventivo (Próximos 30 días)
  const hoy = new Date();
      ...
      <div className='p-4'>
        {vista === 'hub' && (
          <div className='space-y-4'>
            ...
            <div className='flex justify-between items-center mb-2'><div className='flex items-center gap-2 text-gray-600'><Building2 size={20} /><h2 className='text-xl font-bold text-gray-800'>Multicentros</h2></div>{esAdmin && <button onClick={() => { const n = prompt('Nombre del nuevo Multicentro:'); if (n && n.trim()) { const nc = [...centros, { id: n.trim().toLowerCase().replace(/\s+/g, '_'), nombre: n.trim() }]; setCentros(nc); localStorage.setItem('mis_centros_v74', JSON.stringify(nc)); } }} className='bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center gap-1 text-sm font-bold'><Plus size={18} /> Agregar</button>}</div>
        ...
        {centroActual && vista === 'dashboard' && (
          <div className='space-y-4'>
            ...
            {oficinas[centroActual] && Object.keys(oficinas[centroActual]).length === 0 ? <p className='text-sm text-gray-500 bg-white p-4 rounded-lg text-center'>No hay oficinas asignadas a pisos.</p> : (
              <div className='space-y-3'>{Object.entries(oficinas[centroActual] ? oficinas[centroActual].reduce((acc, o) => { const p = o.piso || 'Sin Piso'; if (!acc[p]) acc[p] = []; acc[p].push(o); return acc; }, {}) : {}).sort(([a],[b]) => a.localeCompare(b)).map(([p, ofs]) => (
                <div key={p} className='bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden'>
                  <button onClick={() => setPisoExpandido(pisoExpandido === p ? null : p)} className='w-full p-4 flex justify-between items-center active:bg-gray-50'><span className='font-bold text-gray-800'>Piso {p}</span><div className='flex items-center gap-3'><span className='text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full font-bold'>{datosCentro.filter(a => a.piso === p).length} equipos</span><ChevronDown size={20} className={'text-gray-400 transition-transform ' + (pisoExpandido === p ? 'rotate-180' : '')} /></div></button>
                  {pisoExpandido === p && (<div className='p-3 pt-0 grid grid-cols-2 gap-3'>{ofs.map(o => { const count = datosCentro.filter(a => a.oficina === o.nombre).length; return (<button key={o.id} onClick={() => { setOficinaFiltro(o.nombre); setPisoFiltro(p === 'Sin Piso' ? '' : p); setEstadoFiltro(null); setSubtipoFiltro('Todos'); setVista('lista'); setBusqueda(''); }} className='bg-gray-50 p-3 rounded-lg border border-gray-200 text-left active:bg-gray-100 hover:border-blue-400 transition'><h4 className='font-bold text-gray-800 text-sm'>{o.nombre}</h4><p className='text-xs text-gray-500 mt-1'>{count} equipos</p></button>); })}</div>)}
                </div>
              ))}</div>
            )}
          </div>
        ...
        {vista === 'formulario' && esAdmin && <FormularioActivo activo={editando} guardarDatos={guardarDatos} setVista={setVista} handleVolver={handleVolver} getNextNumber={getNextNumber} centroActual={centroActual} categoriaVista={categoriaVista} oficinas={oficinas} centros={centros} setMsg={setMsg} guardarArchivoNativo={guardarArchivoNativo} escanearParaCampo={async (c) => { try { const image = await Camera.getPhoto({ quality: 80, allowEditing: false, resultType: CameraResultType.Base64, source: CameraSource.Camera }); const img = new Image(); img.onload = () => { const tc = document.createElement('canvas'); tc.width = img.width; tc.height = img.height; const ctx = tc.getContext('2d'); ctx.drawImage(img, 0, 0); const code = jsQR(ctx.getImageData(0,0,tc.width,tc.height).data, tc.width, tc.height); if(code) setForm(prev => ({...prev, [c]: code.data})); }; img.src = 'data:image/jpeg;base64,' + image.base64String; } catch(e){} }} />}
...
function FormularioActivo({ activo, guardarDatos, setVista, handleVolver, getNextNumber, centroActual, categoriaVista, oficinas, centros, setMsg, guardarArchivoNativo, escanearParaCampo }) {
  ...
  const [form, setForm] = useState(activo || { id: Date.now().toString(), centro: centroActual, numero: getNextNumber(), tipo: tipoDefault, nombreEquipo: '', marca: '', modelo: '', codigoActivo: '', numeroSerie: '', procesador: '', generacion: '', ram: '', estado: 'Activo', enAlmacen: esAlmacen, oficina: '', piso: '', personaAsignada: '', fechaProxMantenimiento: '', historial: [] });
  ...
  const handleSubmit = (e) => { e.preventDefault(); guardarDatos(JSON.parse(localStorage.getItem('activos_fijos_v74')||'[]').map(a => a.id === form.id ? form : a)); handleVolver(); };
